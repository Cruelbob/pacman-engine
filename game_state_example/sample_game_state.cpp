...
void sample_game_state::initialize() {
    ...
    auto surface2 = SDL_LoadBMP("loading_text.bmp");
    pacman::size_in_pixels_t image_size2(surface2->w,surface2->h);
    size = (size_t)image_size2.width*image_size2.height*3;
    std::vector<uint8_t> img_buffer2((uint8_t*)surface2->pixels,(uint8_t*)surface2->pixels+size);
